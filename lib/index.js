...
import { isServer, window, document, isClient } from "./platform"
import { clientOnly, noop, jsprefix, cssprefix } from "./utils"


const createDomElement = (x) =>
...
const Popover = createReactClass({
  ...
  /**
   * Returns the props container element or document.body
   * @return {HTMLElement}
   */
  getPopoverContainer () {
    return this.props.containerElement || document.body
  },
  componentWillMount () {
  ...
  componentDidMount () {
    ...
    appendElement(this.layerContainerNode, this.getPopoverContainer())
    this.renderLayer()
  ...
  renderLayer () {
    const { className = "", style = {}, tipSize, body} = this.props
    return ReactDOM.render(createElement(PopoverContent, {
      className: className,
      style: style,
      tipSize: tipSize,
      body: body,
      ...
      ref: (context) => {
        this.containerEl = context ? findDOMNode(context) : null

        if (this.waitingForEnter && context !== null) {
          this.enterLogic()
        }
      }
